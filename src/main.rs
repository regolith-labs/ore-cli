...
async fn main() {
    ...
    match args.command {
        ...
        Commands::UpdateAdmin(cmd) => {
            miner.update_admin(cmd.new_admin).await;
        }
        #[cfg(feature = "admin")]
        Commands::UpdateDifficulty(cmd) => {
            miner.update_difficulty(cmd.new_difficulty).await;
        }
    }
}
...
impl<'a> Miner<'a> {
    ...
    #[cfg(feature = "admin")]
    async fn update_admin(&self, new_admin: String) {
    ...
    #[cfg(feature = "admin")]
    async fn update_difficulty(&self, new_difficulty: String) {
        let new_difficulty = KeccakHash::from_str(new_difficulty.as_str()).unwrap();
        let client =
            RpcClient::new_with_commitment(self.cluster.clone(), CommitmentConfig::processed());
