...
impl Miner {
    async fn stake_migrate(
        ...
        for mint in boost_mints {
            ...
            let legacy_boost_address = state::boost_pda(mint).0;
            let legacy_stake_address =
                state::stake_pda(pubkey, legacy_boost_address).0;
            ...
            {
                if legacy_stake_account.balance > 0 && ask_confirm(
                        format!(
                        ...
                        use ore_boost_api::sdk;
                    ixs.push(sdk::withdraw(pubkey, mint, legacy_stake_account.balance));
        ...
        println!("\n{}", "Scanning pool stake accounts...".bold());
        ...
        for (pool_address, pool) in pools {
            ...
            for mint in boost_mints {
                ...
                if let Ok(share) = get_share(&self.rpc_client, share_address).await {
                    ...
                    if share.balance > 0 && ask_confirm(
                            format!(
